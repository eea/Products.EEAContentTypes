...
class SendAsNotification(object):
    ...
    def __call__(self):
        ...
        res = cat.searchResults(portal_type = 'NewsletterTheme')[0]
        notif_center = res.getObject()
        ...
        for nl in notif_center.getNewsletters():
            ...
            if status == 'published':
                status = '<span style="color:green">notification was sent</span>'
            else:
                status = '<span style="color:red">notification not yet sent</span>'
            ...
            mapping = {u'url' : notif_center.absolute_url(), u'status' : status})
            pu.addPortalMessage(message, 'structure')
            return self.request.RESPONSE.redirect(self.context.absolute_url())

        #Create email notification object
        notif_center.invokeFactory('Newsletter',id=obj_id, title=obj_title, relatedItem=obj_uid)
